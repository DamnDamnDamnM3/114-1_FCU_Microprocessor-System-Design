/* ===============================================================
 * Lab 10-2 - Little Green Man II (完整版本)
 * 功能說明: 綠色小人動畫播放系統（支援速度控制和開始/停止功能）
 * 按鍵功能:
 *   - 按鍵 S (5): 開始/停止動畫
 *   - 按鍵 4: 減速（降低動畫播放速度）
 *   - 按鍵 6: 加速（提高動畫播放速度）
...
// speed_index 0: 200 (最慢), 1: 100, 2: 50, 3: 25 (最快)
...
void TMR1_IRQHandler(void)
{
    ...
    // 移動到下一個位置
    scan_index++;
    if (scan_index > 3) scan_index = 0;  // 循環掃描 0-3

    // ========== 任務 2: 按鍵掃描（防彈跳處理）==========
    key_check_counter++;
    if (key_check_counter >= 20) {  // 每 20ms 掃描一次按鍵（50Hz 掃描頻率）
        key_check_counter = 0;
...
void Process_Keys(void)
{
    ...
    if (current_key != 0) {
        switch (current_key) {
            ...
            case 4:  // 按鍵 4（減速）
                if (speed_index > 0) {
                    speed_index--;  // 降低速度索引（數值越小速度越慢）
                }
                animation_counter = 0;  // 重置動畫計數器，立即套用新速度
                break;
                
            case 6:  // 按鍵 6（加速）
                if (speed_index < 3) {
                    speed_index++;  // 提高速度索引（數值越大速度越快）
                }
                animation_counter = 0;  // 重置動畫計數器，立即套用新速度
                break;
        }
