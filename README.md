...
### Lab 10: 綠色小人動畫系統
**檔案**: `Lab-10/`
- **Q1.c**: 基礎版本（僅支援開始/停止功能）
- **Q2.c**: 完整版本（支援速度控制和開始/停止功能）
- **Lance_bmp_first.c**: 點陣圖處理範例程式
- **技術重點**: 雙定時器協同工作、中斷優先權管理、七段顯示器硬體多工掃描、按鍵防彈跳、動畫播放控制、LCD點陣圖顯示

### Lab 11: 時鐘系統與PWM音樂播放
**檔案**: `Lab-11/`
- **Q1.c**: 快速前進時鐘系統（支援時間設定功能）
- **Q2.c**: PWM音樂播放系統（支援動態音量控制）
- **技術重點**: Timer中斷處理、外部中斷處理、LCD圖形繪製、PWM音樂播放、ADC動態音量控制、三角函數計算

### Lab 12: 1A2B猜數字遊戲（UART通訊版）
**檔案**: `Lab-12/`
- **main.c**: 1A2B遊戲主程式（雙人對戰版本）
- **MCU_init.h**: 微控制器初始化設定檔
- **技術重點**: UART雙向通訊、多中斷協同工作、遊戲邏輯設計、狀態機設計、七段顯示器多工掃描、LCD歷史記錄顯示
...
- **v1.5**: 新增Lab 10，包含雙定時器協同工作和動畫控制技術
- **v1.6**: 新增Lab 11-12，包含時鐘系統、PWM音樂播放和UART通訊技術
